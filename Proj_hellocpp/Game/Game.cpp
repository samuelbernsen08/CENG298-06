...
/// @author Samuel Bernsen, bernsen@chapman.edu
	...
	for (int iRow = 0; iRow < this->m_nGridRows; iRow++)
	{
		...
		for (int iColumn = 0; iColumn < this->m_nGridColumns; iColumn++)
		{
			if ('V' == this->a_cGrid[iRow][iColumn])
			{
				...
				/* HINT: else, display a wave character to keep the Battle Boat location secret, '~' */
				if (bShowBoat == true)
				{
					std::cout << 'V' << "|";
				}
...
bool Game::isCloseToTarget(int iRow, int iColumn)
{
	...
	// the target boat. 
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	int differenceRow = std::abs(iRow - this->m_iRowBoat);
	int differenceColumn = std::abs(iColumn - this->m_iColumnBoat);
	if (((differenceRow >= 0) && (differenceRow <= 1)) && ((differenceColumn >= 0) && (differenceColumn <= 1)))
	{
		bClose = true;
	}
