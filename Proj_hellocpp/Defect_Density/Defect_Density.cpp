...
int main()
{
    ...
    try {
        ...
        if (!(nDefects >= 0)) {
            std::string strMessage = "Invalid number of defects: " + std::to_string(nDefects) + "; number of defects must be >= 0; " + " (Loc: " + __FILE__ + ", " + std::to_string(__LINE__) + ")";
        ...
        if (nProgramUnitSize <= 0) {
            std::string strMessage1 = "Invalid program size: " + std::to_string(nProgramUnitSize) + "; Program size should be greater than zero " + " (Loc: " + __FILE__ + ", " + std::to_string(__LINE__) + ")";
            throw std::runtime_error(strMessage1);
        }
